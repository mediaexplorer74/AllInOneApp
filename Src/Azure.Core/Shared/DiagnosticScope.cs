...
namespace Azure.Core.Pipeline
{
    internal readonly struct DiagnosticScope : IDisposable
    {
        ...
        public void SetStartTime(DateTime dateTime) 
            => _activity?.SetStartTime(dateTime);
...
#pragma warning restore 109
    ...
    {
        private static readonly MethodInfo? s_setIdFormatMethod
            = typeof(Activity).GetTypeInfo().GetDeclaredMethod("SetIdFormat");//.GetMethod("SetIdFormat");
        private static readonly MethodInfo? s_getIdFormatMethod 
            = typeof(Activity).GetTypeInfo().GetDeclaredProperty("IdFormat")?.GetMethod;
        private static readonly MethodInfo? s_getTraceStateStringMethod 
            = typeof(Activity).GetTypeInfo().GetDeclaredProperty("TraceStateString")?.GetMethod;
