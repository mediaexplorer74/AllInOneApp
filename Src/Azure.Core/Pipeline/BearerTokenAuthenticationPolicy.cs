...
namespace Azure.Core.Pipeline
{
    ...
    public class BearerTokenAuthenticationPolicy : HttpPipelinePolicy
    {
        ...
        private async /*Value*/Task ProcessAsync(HttpMessage message, ReadOnlyMemory<HttpPipelinePolicy> pipeline, bool async)
        {
            if (message.Request.Uri.Scheme != /*Uri.UriSchemeHttps*/"https")
            {
        ...
        private class AccessTokenCache
        {
            ...
            public async /*Value*/Task<string> GetHeaderValueAsync(HttpMessage message, bool async)
            {
...
#pragma warning restore AZC0111 // DO NOT use EnsureCompleted in possibly asynchronous scope.
                ...
                if (!headerValueTask.IsCompleted)
                {
                    if (async)
                    {
                        await headerValueTask.AwaitWithCancellation(
...
#pragma warning restore AZC0111 // DO NOT use EnsureCompleted in possibly asynchronous scope.
            ...
            private async /*Value*/Task GetHeaderValueFromCredentialInBackgroundAsync(TaskCompletionSource<HeaderValueInfo> backgroundUpdateTcs, HeaderValueInfo info, HttpMessage httpMessage, bool async)
            {
            ...
            private async /*Value*/Task<HeaderValueInfo> GetHeaderValueFromCredentialAsync(HttpMessage message, bool async, CancellationToken cancellationToken)
            {
