...
namespace Azure
{
    ...
    /// </code>
    /// </example>
    public abstract class AsyncPageable<T> : IEnumerable<T> where T : notnull
    {
        /// <summary>
        ...
        public virtual /*async*/ IEnumerator<T> GetAsyncEnumerator(CancellationToken cancellationToken = default)
        {
            //await 
            foreach (Page<T> page in AsPages())//.Configure(false).WithCancellation(cancellationToken))
            {
...
#pragma warning disable 1998 // async function without await
            public override /*async*/ IEnumerable<Page<T>> AsPages(string? continuationToken = default, int? pageSizeHint = default)
